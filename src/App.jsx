...
const DEFAULT_PERMS = {
  staff:     { work:true,  markets:true,  events:false, sales:false, sops:"health", team:false, inbox:true,  chat:true },
  warehouse: { work:true,  markets:true,  events:false, sales:false, sops:"all",    team:false, inbox:true,  chat:true },
  manager:   { work:true,  markets:true,  events:true,  sales:false, sops:"all",    team:false, inbox:true,  chat:true },
  admin:     { work:true,  markets:true,  events:true,  sales:true,  sops:"all",    team:true,  inbox:true,  chat:true },
};
...
function Login({ onLogin }) {
  ...
  const sendReset = async () => {
    if (!email.trim()) return setErr("Please enter your email address first.");
    setBusy(true); setErr("");
    try {
      await sendPasswordResetEmail(auth, email.trim());
      setResetSent(true);
    } catch (e) {
      setErr("Could not send reset email. Please check the email address.");
    }
    setBusy(false);
  };
        ...
        {resetSent ? (
          <div style={css.form}>
            <div style={{textAlign:"center",marginBottom:16}}>
              ...
              <div style={{fontFamily:"'DM Serif Display',serif",fontSize:20,color:"#fff",marginBottom:8}}>Check your email</div>
              <div style={{fontSize:14,color:T.muted,lineHeight:1.6}}>We sent a password reset link to <b style={{color:"#fff"}}>{email}</b>. Click the link in the email to set a new password.</div>
            </div>
            <button style={{...css.savebtn,width:"100%",padding:12,fontSize:15}} onClick={()=>{setResetSent(false);setResetMode(false);}}>
              Back to Sign In
            </button>
          </div>
        ) : resetMode ? (
          <div style={css.form}>
            <div style={css.ftitle}>Reset Password</div>
            <div style={{fontSize:13,color:T.muted,marginBottom:12}}>Enter your email and we'll send you a link to reset your password.</div>
            <label style={css.lbl}>Email</label>
            <input style={css.inp} type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="your@email.com" onKeyDown={e=>e.key==="Enter"&&sendReset()}/>
            {err && <div style={{fontSize:13,color:"#FF6B6B",marginTop:8,textAlign:"center"}}>{err}</div>}
            <button style={{...css.savebtn,width:"100%",padding:12,marginTop:16,fontSize:15,opacity:busy?0.7:1}} onClick={sendReset} disabled={busy}>
              {busy ? "Sending..." : "Send Reset Email"}
            </button>
            <button style={{...css.cancelbtn,width:"100%",padding:12,marginTop:8,fontSize:14,textAlign:"center"}} onClick={()=>{setResetMode(false);setErr("");}}>
              Back to Sign In
            </button>
          </div>
        ) : (
          <div style={css.form}>
            ...
            <button style={{background:"none",border:"none",color:T.amber,fontSize:13,cursor:"pointer",width:"100%",textAlign:"center",marginTop:12,fontFamily:"'DM Sans',sans-serif"}} onClick={()=>{setResetMode(true);setErr("");}}>
              Forgot Password?
            </button>
          </div>
        )}
        <div style={{textAlign:"center",fontSize:12,color:T.muted,marginTop:12}}>Contact Marc if you need your login credentials.</div>
      </div>
    </div>
  );
}
...
// ── PERMISSIONS EDITOR ────────────────────────────────────────────────────────
function PermissionsEditor({ emps }) {
  const [perms, setPerms] = useState(null);
  const [saved, setSaved] = useState(false);

  useEffect(() => {
    dbGet("rolePermissions").then(d => setPerms(d || JSON.parse(JSON.stringify(DEFAULT_PERMS))));
  }, []);

  const savePerms = async p => {
    setPerms(p);
    await dbSet("rolePermissions", p);
    setSaved(true);
  ...
  const toggle = (role, tab) => {
    ...
    if (tab === "sops") {
      p[role].sops = p[role].sops === "all" ? "health" : p[role].sops === "health" ? false : "all";
    } else {
      p[role][tab] = !p[role][tab];
    }
    savePerms(p);
  };

  if (!perms) return <div style={{padding:32,textAlign:"center",color:T.muted}}>Loading...</div>;

  const ROLES_LIST = [{v:"staff",l:"Staff",color:"#7A9BB5"},{v:"warehouse",l:"Warehouse",color:"#2ECC71"},{v:"manager",l:"Manager",color:"#F5A623"},{v:"admin",l:"Admin",color:"#E74C3C"}];
  const TABS_INFO = [
    {key:"work",    label:"Work Tasks",          icon:"✅"},
    {key:"markets", label:"Markets Checklist",   icon:"🛖"},
    {key:"events",  label:"Events",              icon:"📅"},
    {key:"sales",   label:"Sales",               icon:"💰"},
    {key:"sops",    label:"SOPs",                icon:"📄"},
    {key:"inbox",   label:"Inbox",               icon:"📬"},
    {key:"chat",    label:"Ask (AI Chat)",        icon:"🤖"},
  ];

  return (
    <div>
      <div style={{...css.card,marginBottom:12}}>
        <div style={{fontFamily:"'DM Serif Display',serif",fontSize:15,color:"#fff",marginBottom:4}}>Role Permissions</div>
        <div style={{fontSize:12,color:T.muted,marginBottom:12}}>Tap any cell to toggle access on or off. SOPs cycles through: Health Only → All → None.</div>
        {saved && <div style={{fontSize:12,color:T.green,marginBottom:8,fontWeight:600}}>✓ Saved!</div>}

        {/* Header */}
        <div style={{display:"grid",gridTemplateColumns:"1fr repeat(4,1fr)",gap:4,marginBottom:6}}>
          <div/>
          {ROLES_LIST.map(r=>(
            <div key={r.v} style={{fontSize:11,fontWeight:700,color:r.color,textAlign:"center",textTransform:"uppercase",letterSpacing:"0.04em"}}>{r.l}</div>
          ))}
        </div>

        {/* Permission rows */}
        {TABS_INFO.map(tab=>(
          <div key={tab.key} style={{display:"grid",gridTemplateColumns:"1fr repeat(4,1fr)",gap:4,padding:"6px 0",borderTop:"1px solid #1E2E42",alignItems:"center"}}>
            <div style={{fontSize:13,color:"#fff"}}>{tab.icon} {tab.label}</div>
            {ROLES_LIST.map(r=>{
              const val = perms[r.v]?.[tab.key];
              const isAdmin = r.v === "admin";
              let label, bg, color;
              if (tab.key === "sops") {
                if (val === "all")    { label="All";    bg="#0A2010"; color=T.green; }
                else if (val === "health") { label="Health"; bg="#1A2A6C"; color="#7EC8E3"; }
                else                  { label="None";   bg="#2A0A0A"; color="#FF6B6B"; }
              } else {
                if (val) { label="✓"; bg="#0A2010"; color=T.green; }
                else     { label="—"; bg="#1E2535"; color:"#3A4A5A"; }
              }
              return(
                <button key={r.v}
                  onClick={()=>!isAdmin&&toggle(r.v,tab.key)}
                  disabled={isAdmin}
                  style={{background:bg,color:color,border:`1px solid ${color}22`,borderRadius:6,padding:"5px 2px",fontSize:12,fontWeight:700,cursor:isAdmin?"default":"pointer",textAlign:"center",fontFamily:"'DM Sans',sans-serif",opacity:isAdmin?0.6:1}}>
                  {label}
                </button>
              );
            })}
        ...
        <div style={{fontSize:11,color:T.muted,marginTop:10}}>Admin always has full access and cannot be changed.</div>
      </div>

      {/* Team roster */}
      <div style={css.card}>
        <div style={{fontFamily:"'DM Serif Display',serif",fontSize:15,color:"#fff",marginBottom:8}}>Your Team</div>
        {emps.map(e=>{
          const roleInfo = ROLES_LIST.find(r=>r.v===(e.role||"staff"));
          return(
            <div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderTop:"1px solid #1E2E42"}}>
              <div style={{width:28,height:28,borderRadius:"50%",background:e.color,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:12,fontWeight:700,flexShrink:0}}>{e.name[0]}</div>
              <div style={{flex:1}}><div style={{fontSize:14,color:"#fff"}}>{e.name}</div></div>
              <span style={{...css.badge,background:roleInfo?.color+"22",color:roleInfo?.color||T.muted}}>{e.role||"staff"}</span>
            </div>
          );
        })}
      </div>
...
function TeamView({ emps, saveEmps, isAdmin, authUser }) {
  ...
  const add=()=>{
    ...
    saveEmps([...emps,{id:uid(),name:name.trim(),email:email.trim().toLowerCase(),color:COLS[emps.length%COLS.length],role:"staff",noteMode:"toggle"}]);
  ...
  const TABS_INFO = [
    ...
    {key:"markets",  label:"Markets Checklist"},
    {key:"events",   label:"Events"},
    {key:"sales",    label:"Sales"},
    {key:"sops",     label:"SOPs"},
    {key:"inbox",    label:"Inbox"},
    {key:"chat",     label:"Ask (AI Chat)"},
  ];
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:6,marginBottom:16}}>
        <button style={{...css.stab,...(subTab==="members"?css.stabon:{})}} onClick={()=>setSubTab("members")}>Members</button>
        <button style={{...css.stab,...(subTab==="permissions"?css.stabon:{})}} onClick={()=>setSubTab("permissions")}>Permissions</button>
      </div>
      ...
      {subTab==="members"&&<>
        {showAdd&&isAdmin&&<div style={css.form}>
          ...
          <input style={css.inp} value={name} onChange={e=>setName(e.target.value)} placeholder="First Last"/>
          <label style={css.lbl}>Email * (used for login)</label>
          <input style={css.inp} type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="employee@email.com"/>
          ...
          <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
            <button style={css.cancelbtn} onClick={()=>setShowAdd(false)}>Cancel</button>
            <button style={css.savebtn} onClick={add}>Add</button>
          </div>
        </div>}
        {emps.map(e=>(
          <div key={e.id} style={{...css.form,marginBottom:8}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontSize:15,fontWeight:600,color:"#fff"}}>{e.name}</div>
                <div style={{fontSize:12,color:T.muted}}>{e.email}</div>
                <span style={{...css.badge,background:"#1A2A6C",color:"#7EC8E3",marginTop:4,display:"inline-block"}}>{e.role||"staff"}</span>
              </div>
      ...
      {/* PERMISSIONS TAB */}
      {subTab==="permissions"&&<PermissionsEditor emps={emps}/>}
    </div>
  );
}
