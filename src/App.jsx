...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [e, st, so, tp, as, sd] = await Promise.all([
  ...
  return (
    <>
      ...
      <div style={css.app}>
        ...
        <nav style={css.nav}>
          {[["work","Work"],["markets","Markets"],["sops","SOPs"],["team","Team"],["chat","Ask"]].map(([id,l]) => (
...
function ChatView({ sops, steps, tpls }) {
  ...
  const send=async()=>{
    ...
    try{
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1200,system:ctx(),tools:[{type:"web_search_20250305",name:"web_search"}],messages:nm.map(m=>({role:m.role,content:m.content}))})});
