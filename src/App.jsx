...
function TeamView({ emps, saveEmps, isAdmin, authUser }) {
  ...
  return(
    <div>
      ...
      {subTab==="permissions"&&<>
        <div style={{...css.card,marginBottom:12}}>
          <div style={{fontFamily:"'DM Serif Display',serif",fontSize:15,color:"#fff",marginBottom:8}}>Role Permissions</div>
          <div style={{fontSize:12,color:T.muted,marginBottom:12}}>Shows what each role can access. Change an employee's role in the Members tab.</div>

          {/* Header row */}
          <div style={{display:"grid",gridTemplateColumns:"1fr repeat(4, 60px)",gap:4,marginBottom:8}}>
            <div style={{fontSize:11,fontWeight:700,color:T.muted}}></div>
            {ROLES.map(r=><div key={r.v} style={{fontSize:11,fontWeight:700,color:"#7EC8E3",textAlign:"center",textTransform:"uppercase"}}>{r.l}</div>)}
          </div>

          {/* Permission rows */}
          {TABS_INFO.map(tab=>(
            <div key={tab.key} style={{display:"grid",gridTemplateColumns:"1fr repeat(4, 60px)",gap:4,padding:"8px 0",borderTop:"1px solid #1E2E42",alignItems:"center"}}>
              <div style={{fontSize:13,color:"#fff"}}>{tab.label}</div>
              {ROLES.map(r=>{
                const perms = DEFAULT_PERMS[r.v];
                const hasAccess = tab.key==="sops" ? perms.sops!==false : perms[tab.key];
                const label = tab.key==="sops" ? (perms.sops==="all"?"All":"Health") : (hasAccess?"✓":"—");
                return(
                  <div key={r.v} style={{textAlign:"center",fontSize:13,fontWeight:600,color:hasAccess?T.green:"#3A3A4A"}}>
                    {label}
                  </div>
                );
              })}
        ...
        <div style={{...css.card}}>
          ...
          {emps.map(e=>(
            <div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderTop:"1px solid #1E2E42"}}>
              ...
              <span style={{...css.badge,background:"#1A2A6C",color:"#7EC8E3"}}>{e.role||"staff"}</span>
            </div>
          ))}
        </div>
      </>}
